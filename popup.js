  ...
  if (userInput) {
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
      chrome.scripting.executeScript({
        ...
        chrome.tabs.sendMessage(tabs[0].id, { action: 'getFormQuestionsAndInputs' }, (response) => {
          if (chrome.runtime.lastError) {
          ...
          if (response && response.formData) {
            ...
            getChatGPTResponse(userInput, formData).then(answers => {
              ...
              chrome.tabs.sendMessage(tabs[0].id, { action: 'fillForm', answers: answers }, (response) => {
...
async function getChatGPTResponse(userInput, formData) {
  ...
  while (retries > 0) {
    try {
      console.log('Sending request to OpenAI:', { userInput, formData });
      const response = await fetch('https://api.openai.com/v1/chat/completions', {
        ...
        body: JSON.stringify({
          model: 'gpt-3.5-turbo',
          messages: [
            { role: 'system', content: 'You are an assistant that fills out form fields based on user input.' },
            { role: 'user', content: `User input: ${userInput}` },
            ...formData.map(data => ({ role: 'user', content: `Form question: ${data.question}` }))
          ],
          max_tokens: 150,
          temperature: 0.7
        })
